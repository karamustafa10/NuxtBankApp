      ...
      <div class="card">
        <div class="card__info">
          <div class="card__logo">
            {{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.cardName
                : getCardWithIdStore.cards[1]?.cardName
            }}
          </div>
          ...
          <div class="card__type">
            {{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.type
                : getCardWithIdStore.cards[1]?.type
            }}
          </div>
          <div class="card__number">
            <span class="card__digit-group">{{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.cardNumber.slice(0, 4)
                : getCardWithIdStore.cards[1]?.cardNumber.slice(0, 4)
            }}</span>
            <span class="card__digit-group">{{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.cardNumber.slice(4, 8)
                : getCardWithIdStore.cards[1]?.cardNumber.slice(4, 8)
            }}</span>
            <span class="card__digit-group">{{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.cardNumber.slice(8, 12)
                : getCardWithIdStore.cards[1]?.cardNumber.slice(8, 12)
            }}</span>
            <span class="card__digit-group">{{
              getCardWithIdStore.cards[0]?.type == "debit"
                ? getCardWithIdStore.cards[0]?.cardNumber.slice(12, 16)
                : getCardWithIdStore.cards[1]?.cardNumber.slice(12, 16)
            }}</span>
          </div>
          ...
          <div class="card__exp-date">
            <time datetime="2038-01">
              {{
                getCardWithIdStore.cards[0]?.type == "debit"
                  ? getCardWithIdStore.cards[0]?.validDate
                      .toString()
                      .slice(5, 7) +
                    "/" +
                    getCardWithIdStore.cards[0]?.validDate
                      .toString()
                      .slice(2, 4)
                  : getCardWithIdStore.cards[1]?.validDate
                      .toString()
                      .slice(5, 7) +
                    "/" +
                    getCardWithIdStore.cards[1]?.validDate
          ...
          <div class="card__name" aria-label="Dee Stroyer">
            {{
              getUserWithIdStore.user?.firstName +
              " " +
              getUserWithIdStore.user?.lastName
            }}
          </div>
...
onMounted(async () => {
  await Promise.all([
    ...
    getCardWithIdStore.fetchCardWithId(),
    getUserWithIdStore.fetchUserWithId(),
  ]);
});
</script>
...
.card {
  ...
  background-color: hsl(212, 99%, 49%);
  /* Örnek olarak mavi tonları */
